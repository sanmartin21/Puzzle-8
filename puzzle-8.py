...
# Função para exibir o estado do quebra-cabeça


def exibir_estado(estado):
    # Tamanho de cada peça em pixels
    tamanho_peca = 100

    # Criar uma imagem grande para o quebra-cabeça completo
    largura_total = tamanho_peca * 3
    altura_total = tamanho_peca * 3
    ...
    for i in range(3):
        for j in range(3):
            peca = estado[i][j]
            if peca is not None:
                # Carregue a imagem correspondente à peça
                imagem_peca = Image.open(f'{peca}.png')
                # Cole a imagem da peça na posição correta na imagem completa
                x = j * tamanho_peca
                y = i * tamanho_peca
                imagem_completa.paste(imagem_peca, (x, y))

    # Mostre a imagem completa
    imagem_completa.show()
...
if __name__ == "__main__":
    ...
    # Chame exibir_estado para mostrar o estado inicial ou qualquer outro estado do quebra-cabeça
    exibir_estado(puzzle.estado_inicial)
    metodo = int(input(
